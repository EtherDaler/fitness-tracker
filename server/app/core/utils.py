...
async def insert_generated_workouts(user: User):
    ...
    workout3 = Workout(
        name="Программа для мышц ног и ягодиц",
        description="auto-generated",
        user_id=user.id,
    )
    db.add(workout3)
    await db.commit()
    await db.refresh(workout3)
    ...
    workout4 = Workout(
        name="Программа  на все тело",
        description="auto-generated",
        user_id=user.id,
    )
    db.add(workout4)
    await db.commit()
    await db.refresh(workout4)
